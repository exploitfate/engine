...
class EngineQueue
{
    ...
    public function __construct(EngineQueueConnectionConfig $queueConnectionConfig = null)
    {
        if ($queueConnectionConfig !== null) {
            $this->user = $queueConnectionConfig->getUser();
            $this->password = $queueConnectionConfig->getPassword();
            $this->host = $queueConnectionConfig->getHost();
            $this->port = $queueConnectionConfig->getPort();
            $this->virtualHost = $queueConnectionConfig->getVirtualHost();
        }
    }
     ...
     *
     * @param string $queueName EngineQueue names may be up to 255 bytes of UTF-8 characters
     * @param array|mixed $data
    ...
    public function publish($queueName, $data, $exchange = '', $delay = 0)
    {
        ...
        /**
         * @param queue string EngineQueue names may be up to 255 bytes of UTF-8 characters
         ...
         * @param auto_delete boolean EngineQueue is deleted when last consumer unsubscribes
         */
